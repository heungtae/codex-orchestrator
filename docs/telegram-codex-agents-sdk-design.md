...
- 명령 파싱:
  ...
  - 강제 Codex 전달: `/codex /...` (예약 명령과 충돌할 때 사용)
...
- `/codex /...`: Codex 슬래시 명령 강제 전달 (예: `/codex /status`)
...
3. 입력이 `/codex /...` 형태면 `/codex ` 접두어를 제거 후 Codex로 전달
4. 실행 결과를 일반 요청과 동일하게 응답/trace 저장
...
- 지원하지 않는 Codex 슬래시 명령 입력: 에러 메시지와 함께 사용 가능한 전달 형식(`/codex /...`) 안내
